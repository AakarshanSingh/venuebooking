...
app.post("/booking", async (req, res) => {
  try {
      ...
      $and: [
        ...
        },
        {
          $or: [
            {
              $and: [
                { TimeInSecFrom: { $gt: TimeInSecTo } },
                { TimeInSecTo: { $lt: TimeInSecFrom } },
              ],
            },
            {
              $and: [
            ...
            {
              $and: [
                { TimeInSecFrom: { $gt: TimeInSecFrom } },
                { TimeInSecTo: { $lt: TimeInSecTo } },
              ],
            },
            {
              $and: [
                { TimeInSecFrom: { $lt: TimeInSecFrom } },
                { TimeInSecTo: { $gt: TimeInSecTo } },
              ],
            },
          ],
        },
    ...
    if (newbo === 0) {
      ...
      res.status(201).render("venuebooking", {
        success: "Venue is currently booked for this interval.",
      });
    }
  } catch (error) {
